...
const Game = function (name, host) {
  ...
  this.assignBlind = () => {
    ...
    const goFirstIndex =
      ...
      this.roundData.bigBlind >= this.players.length - 1 ? 0 : this.roundData.bigBlind;

    let x = goFirstIndex > this.players.length - 1 ? 0 : goFirstIndex;
    this.roundData.turn = this.players[x].getUsername();
    this.players[x].setStatus('Their Turn');
  };
    ...
    if (this.roundNum == 0) {
      ...
      this.roundData.dealer =
        this.roundData.dealer + 1 < this.players.length
          ...
          : 0;
      this.roundData.dealer =
        this.roundData.dealer >= this.players.length
          ? 0
          : this.roundData.dealer;
    }
  ...
  this.moveOntoNextPlayer = () => {
    let handOver = false;
    console.log("this.isStageComplete()", this.isStageComplete())
    if (this.isStageComplete()) {
      ...
      if (!handOver && numNonFolds == 1) {
        ...
        let count = 0;
        do {
          currTurnIndex = currTurnIndex + 1;
          currTurnIndex = currTurnIndex >= this.players.length ? 0 : currTurnIndex;
          // count ++;
        } while (
